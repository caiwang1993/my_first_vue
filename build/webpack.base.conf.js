...
//下面为webpack基本的配置信息（可以理解成是开发环境和生产环境的公共配置）
module.exports = {
  //webpack解析文件时候的根目录（如果把webpack.base.conf.js放在项目的根目录下面，这个配置可以省略
  ...
  output: {
    ...
    //生成文件的名称
    filename: '[name].js',
    //输出解析文件的目录，url相当于HTML页面（生成的html文件中，css和js等静态文件的url前缀）
  ...
  module: {
    // rules是一个数组，其中的每一个元素都是对象，这个对象是针对具体类型的文件进行配置
    rules: [
      ...
      },
      {
        ...
        loader: 'url-loader',
        options: {
          //文件的大小小于10000字节(10k)的时候会返回一个dataUrl
          limit: 10000,
          //生成的文件的保存路径和后缀名称
